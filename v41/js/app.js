...
(() => {
  ...
  const productShareToast = (res) => {
    if (res === 'error') return;
    if (res === 'cancelled') return toast('Share cancelled');
    if (res === 'shared') return toast('Caption copied — paste it as the photo description');
    if (res === 'shared-text') return toast('Shared');
    return toast('Photo saved · caption copied');
  };
  ...
  // Share one product as a single post. Viber/Messenger split a shared photo and
  // text into two messages, but both let you type a description on the photo
  // before sending — so we copy the caption to the clipboard, then share the
  // photo alone. Long-press the description field and Paste = one post, real text.
  // 'shared' | 'shared-text' | 'cancelled' | 'downloaded' | 'error'.
  async function shareOneProduct(p) {
    ...
    try {
      ...
      // Copy the caption first (within the tap gesture) so it's ready to paste.
      ...
      if (isMobileLike() && navigator.share) {
        ...
        try {
          // Photo alone when we have one (caption goes in the app's description
          // field); otherwise there's nothing to attach, so share the text.
          await navigator.share(canFiles ? { files: [file] } : { text: caption });
          return canFiles ? 'shared' : 'shared-text';
        } catch (e) { if (e && e.name === 'AbortError') return 'cancelled'; /* else fall back */ }
      }
      // Desktop / no share: save the photo (caption already on the clipboard).
