...
(() => {
  ...
  // ---------- Per-product share (photo + text caption, for Viber / Messenger) ----------
  const productSlug = (p) => ((p && p.name) || 'product').replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'product';
  const productShareToast = (res) => {
    if (res === 'error') return;
    toast(res === 'shared' ? 'Ready to share'
      : res === 'cancelled' ? 'Share cancelled'
      : 'Photo saved · caption copied');
  };

  // The chat caption that rides alongside the product photo. Mirrors how the
  // shop posts it: title, detail, price + discount, then the stock line.
  ...
  // Share one product as its own post: the actual product photo + a text
  // caption. Returns 'shared' | 'cancelled' | 'downloaded' | 'error'.
  async function shareOneProduct(p) {
    ...
    try {
      if (p.image) {
        ...
        // Prefer sharing photo + caption together (Viber/Messenger post).
        if (isMobileLike() && file && navigator.canShare && navigator.canShare({ files: [file], text: caption })) {
          try { await navigator.share({ files: [file], text: caption }); return 'shared'; }
          catch (e) { if (e && e.name === 'AbortError') return 'cancelled'; }
        }
        // Some targets can't take files + text; try file-only, caption to clipboard.
        if (isMobileLike() && file && navigator.canShare && navigator.canShare({ files: [file] })) {
          try {
        ...
        // Desktop / no file share: download the photo, copy caption to paste.
        downloadBlob(blob, productSlug(p) + '.' + ext);
        try { await navigator.clipboard.writeText(caption); } catch (e) {}
        return 'downloaded';
      }
      // No photo — share the caption text alone.
