...
(() => {
  ...
  // Ten distinct offer DESIGNS. Each id maps to a layout in SHEET_DESIGNS below;
  // c1=accent, c2=deep, bg=sheet background, ink=hero-title color, dark=dark canvas.
  const OFFER_THEMES = [
    { id: 'bold',      name: 'Bold',      c1: '#e11d48', c2: '#7a1030', bg: '#f4f6fb', eyebrow: '★ ONHAND STOCK ★' },
    { id: 'minimal',   name: 'Minimal',   c1: '#2563eb', c2: '#111827', bg: '#ffffff', ink: '#111827', eyebrow: 'NEW ARRIVALS' },
    { id: 'editorial', name: 'Editorial', c1: '#b91c1c', c2: '#1c1917', bg: '#faf7f0', ink: '#1c1917', eyebrow: 'THE COLLECTION' },
    { id: 'noir',      name: 'Noir',      c1: '#d4af37', c2: '#8a6d1a', bg: '#0c0c0e', ink: '#d4af37', dark: true, eyebrow: 'PREMIUM SELECTION' },
    { id: 'blocks',    name: 'Blocks',    c1: '#2563eb', c2: '#f59e0b', bg: '#fffbeb', ink: '#ffffff', eyebrow: 'MEGA DEALS' },
    { id: 'ticket',    name: 'Ticket',    c1: '#ea580c', c2: '#9a3412', bg: '#fff7ed', ink: '#ffffff', eyebrow: 'SPECIAL OFFER' },
    { id: 'neon',      name: 'Neon',      c1: '#22d3ee', c2: '#7c3aed', bg: '#0b1020', ink: '#ffffff', dark: true, eyebrow: '⚡ FLASH SALE ⚡' },
    { id: 'catalog',   name: 'Catalog',   c1: '#0f766e', c2: '#134e4a', bg: '#ffffff', ink: '#ffffff', eyebrow: 'WHOLESALE PRICE LIST' },
    { id: 'pastel',    name: 'Pastel',    c1: '#ec4899', c2: '#a21caf', bg: '#fdf2f8', ink: '#831843', eyebrow: '♥ NEW IN ♥' },
    { id: 'poster',    name: 'Poster',    c1: '#f59e0b', c2: '#166534', bg: '#052e16', ink: '#ffffff', dark: true, eyebrow: 'LIMITED STOCK' },
  ];
  const DEFAULT_THEME = 'bold';
  function resolveTheme(offer) {
    offer = offer || {};
    const byId = OFFER_THEMES.find(t => t.id === offer.themeId);
    if (byId) return byId;
    const acc = offer.accent;
    if (acc) {
      const m = OFFER_THEMES.find(t => t.c1.toLowerCase() === String(acc).toLowerCase());
      if (m) return m;
      return { id: 'bold', name: 'Custom', c1: acc, c2: shade(acc, -0.34), bg: '#f4f6fb', eyebrow: '★ ONHAND STOCK ★' };
    }
    return OFFER_THEMES[0];
  }
  ...
  function duplicateOffer(id) {
    ...
    state.offer = {
      ...
      title: o.title || '',
      footer: o.footer, accent: o.accent, themeId: o.themeId || DEFAULT_THEME,
  ...
  function loadOfferIntoBuilder() {
    ...
    if (!o.themeId) o.themeId = resolveTheme(o).id;
    renderThemePicker();
    renderOfferItems();
  }
  ...
  function renderThemePicker() {
    ...
    const cur = state.offer.themeId || DEFAULT_THEME;
    host.innerHTML = OFFER_THEMES.map(t => `
      <button type="button" class="theme-swatch${t.id === cur ? ' active' : ''}" data-theme-id="${t.id}" aria-label="${t.name} theme">
        <span class="ts-chip" style="background:linear-gradient(135deg, ${t.c1}, ${t.c2})"></span>
        <span class="ts-name">${t.name}</span>
      </button>`).join('');
    $$('.theme-swatch', host).forEach(b => b.addEventListener('click', () => {
      const t = OFFER_THEMES.find(x => x.id === b.dataset.themeId);
      if (!t) return;
      state.offer.themeId = t.id;
      state.offer.accent = t.c1;
      $$('.theme-swatch', host).forEach(x => x.classList.toggle('active', x === b));
    }));
  }
  ...
  async function saveOffer() {
    ...
    const clean = {
      ...
      themeId: state.offer.themeId || DEFAULT_THEME, items: memItems,
  ...
  const s_price = (it) => {
    ...
    return `<div class="s-price"><span class="s-price-val"><span class="s-cur">${cur()}</span><span class="s-single">${fmt(pr.price).replace(cur(), '')}</span></span>${badgeHTML(it, 's-badge', 's-badge s-badge-net', null)}</div>`;
  };
  const s_card = (it) => `<div class="scard"><div class="s-imgbox">${s_img(it)}</div><div class="s-body">${s_desc(it)}${s_price(it)}</div></div>`;
  ...
  // Sections; style 'num' prepends a zero-padded index (editorial)
  function s_sections(c, style) {
    return c.groups.map((g, i) => {
      let banner = '';
      if (g.title) {
        banner = style === 'num'
          ? `<div class="s-banner"><span class="s-num">${String(i + 1).padStart(2, '0')}</span>${esc(g.title)}</div>`
          : `<div class="s-banner">${esc(g.title)}</div>`;
      }
      return `<section class="sgroup">${banner}${s_grid(g)}</section>`;
    }).join('');
  }
  const s_body = (c, style) => `<div class="sheet-body">${s_sections(c, style)}</div>`;
  const s_foot = (c) => `<div class="sheet-footer"><span>${c.footer}</span></div>`;
  // Hero variants
  const heroSplit = (c, cut) => `<div class="hero"><div class="hero-logo">${s_logo(c)}</div><div class="hero-text"><span class="hero-eyebrow">${c.eyebrow}</span><h1 class="hero-title">${c.title}</h1></div>${cut ? '<div class="hero-cut"></div>' : ''}</div>`;
  const heroCenter = (c, extra) => `<div class="hero"><div class="hero-logo">${s_logo(c)}</div><span class="hero-eyebrow">${c.eyebrow}</span><h1 class="hero-title">${c.title}</h1>${extra || ''}</div>`;

  // ---- The ten designs ----
  const SHEET_DESIGNS = {
    bold:      (c) => heroSplit(c, true) + s_body(c) + s_foot(c),
    minimal:   (c) => heroCenter(c) + s_body(c) + s_foot(c),
    editorial: (c) => `<div class="hero"><div class="hero-top"><div class="hero-logo">${s_logo(c)}</div><span class="hero-eyebrow">${c.eyebrow}</span></div><h1 class="hero-title">${c.title}</h1></div>` + s_body(c, 'num') + s_foot(c),
    noir:      (c) => heroCenter(c, '<span class="hero-rule"></span>') + s_body(c) + s_foot(c),
    blocks:    (c) => heroSplit(c) + s_body(c) + s_foot(c),
    ticket:    (c) => heroSplit(c) + s_body(c) + s_foot(c),
    neon:      (c) => heroSplit(c) + s_body(c) + s_foot(c),
    catalog:   (c) => heroSplit(c) + s_body(c) + s_foot(c),
    pastel:    (c) => heroSplit(c) + s_body(c) + s_foot(c),
    poster:    (c) => heroCenter(c) + s_body(c) + s_foot(c),
  };
  ...
  function buildSheet(offer) {
    ...
    const t = c.theme;
    const design = SHEET_DESIGNS[t.id] ? t.id : 'bold';
    const vars = `--accent:${t.c1};--deep:${t.c2};--sheet-bg:${t.bg};--ink:${t.ink || '#fff'}`;
    const cls = 'sheet d-' + design + (t.dark ? ' dark' : '');
    return `<div class="${cls}" style="${vars}">${SHEET_DESIGNS[design](c)}</div>`;
  }
