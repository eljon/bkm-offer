...
(() => {
    ...
    $$('.pcard', grid).forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.pshare-btn')) {
          const p = state.products.find(x => x.id === el.dataset.id);
          if (p) shareOneProduct(p).then((res) => deliverToast(res, 'Product'));
          return;
        }
  ...
  // ---------- Per-product share (image to Viber / Messenger, etc.) ----------
  const productSlug = (p) => ((p && p.name) || 'product').replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'product';

  // A clean, branded single-product card (portrait) for chat apps.
  function productShareHTML(p) {
    const pr = computePricing(p);
    const logo = currentLogo();
    const logoEl = logo
      ? `<img class="pshare-logo-img" src="${logo}" alt="logo">`
      : `<div class="pshare-wordmark">BJ Auto Parts Supply</div>`;
    const img = p.image
      ? `<img src="${p.image}" alt="">`
      : `<span class="pshare-noimg">${icon('box')}</span>`;
    const desc = p.description ? `<p class="pshare-desc">${esc(p.description)}</p>` : '';
    const price = pr.price != null
      ? `<div class="pshare-price"><span class="pshare-cur">${cur()}</span><span class="pshare-amt">${fmt(pr.price).replace(cur(), '')}</span>${badgeHTML(p, 'pshare-badge', 'pshare-badge pshare-badge-net', null)}</div>`
      : '';
    return `<div class="pshare">
      <div class="pshare-top">${logoEl}</div>
      <div class="pshare-imgwrap">${img}</div>
      <div class="pshare-info"><h2 class="pshare-name">${esc(p.name) || 'Product'}</h2>${desc}${price}</div>
      <div class="pshare-foot">LIMITED STOCK ONLY</div>
    </div>`;
  }
  ...
  // Share one product's image. Returns 'shared' | 'cancelled' | 'downloaded'.
  ...
  async function shareProducts(list) {
    ...
    if (list.length === 1) { const r = await shareOneProduct(list[0]); deliverToast(r, 'Product'); return; }
    if (!isMobileLike()) {
      for (const p of list) { await shareOneProduct(p); }
      toast(`${list.length} products downloaded`);
      return;
    }
