...
(() => {
  ...
  function renderProducts() {
    ...
    $$('.pcard', grid).forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        if (!p) return;
        if (e.target.closest('.pcheck')) { toggleProductSelect(p.id); return; }
        if (e.target.closest('.pedit')) { openProductModal(p); return; }
        if (e.target.closest('.pshare-btn')) { shareOneProduct(p).then(productShareToast); return; }
        openProductView(p);   // tap the card → detail highlight modal
      });
    });
    if (canDrag) {
      makeSortable(grid, {
  ...
  function productCardHTML(p, selected, showEdit, drag) {
    ...
    // Library view (showEdit): a checkbox selects, the card body opens a detail
    // view, and edit/share sit in a bottom row (no longer overlapping the text).
    const check = showEdit
      ? `<button class="pcheck" data-act="check" role="checkbox" aria-checked="${selected ? 'true' : 'false'}" aria-label="Select"><span class="pcheck-box">${icon('check')}</span></button>`
      : '';
    const actions = showEdit
      ? `<div class="pactions">
          <button class="pact pedit" data-act="edit" aria-label="Edit">${icon('edit')}</button>
          <button class="pact pshare-btn" data-act="share" aria-label="Share">${icon('share')}</button>
        </div>`
      : '';
    return `<div class="pcard ${selected ? 'selected' : ''}" data-id="${p.id}">
      ${check}${dragBtn}
      <div class="pcard-tap" data-act="view">
        ${img}
        <div class="pbody">
  ...
  // Tap a product → a highlight card in a modal (view only; selection is via
  // the checkbox). Share / Edit are available right there.
  let viewingProductId = null;
  function openProductView(p) {
    viewingProductId = p.id;
    const pr = computePricing(p);
    const img = p.image
      ? `<div class="pv-photo"><img src="${p.image}" alt=""></div>`
      : `<div class="pv-photo pv-ph">${icon('box')}</div>`;
    const desc = p.description ? `<p class="pv-desc">${esc(p.description)}</p>` : '';
    let price = '';
    if (pr.price != null) {
      const less = lessPct(p);
      const sub = less != null ? `<span class="pv-less">LESS ${less}%</span>` : `<span class="pv-less pv-net">NET</span>`;
      price = `<div class="pv-price"><span class="pv-cur">${cur()}</span><span class="pv-amt">${fmt(pr.price).replace(cur(), '')}</span>${sub}</div>`;
    }
    $('#productViewBody').innerHTML = `<div class="pv-card">
      ${img}
      <div class="pv-info">
        <h2 class="pv-name">${esc(p.name) || 'Untitled'}</h2>
        ${desc}
        ${price}
        <p class="pv-stock">Limited stock only</p>
      </div>
    </div>`;
  ...
  // ---------- Per-product share (single composite image for Viber / Messenger) ----------
  // The photo + details are baked into ONE image so it always lands as a single
  // chat post — no separate caption message and nothing to paste.
  const productSlug = (p) => ((p && p.name) || 'product').replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'product';
  const productShareToast = (res) => {
    if (res === 'error') return;
    toast(res === 'shared' ? 'Ready to share' : res === 'cancelled' ? 'Share cancelled' : 'Image saved');
  };

  // One image: product photo on top, details beneath it — like the shop's posts.
  function productShareHTML(p) {
    const pr = computePricing(p);
    const photo = p.image ? `<div class="ps-photo"><img src="${p.image}" alt=""></div>` : '';
    const desc = p.description ? `<p class="ps-desc">${esc(p.description)}</p>` : '';
    let price = '';
    if (pr.price != null) {
      const less = lessPct(p);
      const sub = less != null ? `<div class="ps-less">LESS ${less}%</div>` : `<div class="ps-less ps-net">NET PRICE</div>`;
      price = `<div class="ps-price"><span class="ps-cur">${cur()}</span><span class="ps-amt">${fmt(pr.price).replace(cur(), '')}</span></div>${sub}`;
    }
    const logo = currentLogo();
    const brand = logo
      ? `<img class="ps-logo" src="${logo}" alt="logo">`
      : `<span class="ps-word">BJ Auto Parts Supply</span>`;
    return `<div class="pshare">
      ${photo}
      <div class="ps-body">
        <h2 class="ps-title">${esc(p.name) || 'Product'}</h2>
        ${desc}
        ${price}
        <p class="ps-stock">Limited stock only</p>
      </div>
      <div class="ps-brand">${brand}</div>
    </div>`;
  }

  async function makeProductBlob(p) {
    const wrap = document.createElement('div');
    wrap.style.cssText = 'position:fixed;left:-10000px;top:0;';
    wrap.innerHTML = productShareHTML(p);
    document.body.appendChild(wrap);
    try {
      const canvas = await html2canvas(wrap.querySelector('.pshare'), { scale: 2, useCORS: true, backgroundColor: '#ffffff' });
      return await new Promise((res) => canvas.toBlob(res, 'image/png'));
    } finally { wrap.remove(); }
  }

  // Share one product as a single image post. 'shared' | 'cancelled' | 'downloaded' | 'error'.
  async function shareOneProduct(p) {
    try {
      const blob = await makeProductBlob(p);
      return await deliverFile(blob, productSlug(p) + '.png', 'image/png');
  ...
  function bind() {
    ...
    $('#pvShareBtn').addEventListener('click', () => {
      const p = state.products.find(x => x.id === viewingProductId);
