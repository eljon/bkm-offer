...
(() => {
  ...
  function makeSortable(container, { itemSel, handleSel, idAttr, grid, onDrop }) {
    $$(handleSel, container).forEach((handle) => {
      ...
      handle.addEventListener('pointerdown', (e) => {
        ...
        const parent = dragEl && dragEl.parentElement;
        if (!dragEl || !parent) return;
        ...
        const onMove = (ev) => {
          ...
          if (!target || target === dragEl || target.parentElement !== parent) return;
          const r = target.getBoundingClientRect();
          let before;
          if (grid) {
            const midY = r.top + r.height / 2;
            before = ev.clientY < r.top + r.height * 0.28 ? true
              : ev.clientY > r.bottom - r.height * 0.28 ? false
                : ev.clientX < r.left + r.width / 2;
            if (Math.abs(ev.clientY - midY) < 2) before = ev.clientX < r.left + r.width / 2;
          } else {
            before = ev.clientY < r.top + r.height / 2;
          }
          parent.insertBefore(dragEl, before ? target : target.nextSibling);
        };
        const onUp = () => {
          ...
          const ids = $$(':scope > ' + itemSel, parent).map((el) => el.getAttribute(idAttr));
          onDrop(ids, parent);
        };
  ...
  const DRAG_HANDLE = '<button class="drag-handle" type="button" aria-label="Hold and drag to reorder">⠿</button>';

  function closeAllSortMenus() { $$('.sort-menu').forEach((m) => m.classList.add('hidden')); }
  function updateSortBtn(btnSel, menuSel, val) {
    const menu = $(menuSel);
    if (menu) $$('button[data-sort]', menu).forEach((b) => b.classList.toggle('active', b.dataset.sort === val));
  }
  ...
  function productCardHTML(p, selected, showEdit, drag) {
    ...
    return `<div class="pcard ${selected ? 'selected' : ''}" data-id="${p.id}">
      <div class="check">✓</div>${editBtn}${dragBtn}
      ${img}
      <div class="pbody">
    ...
    list.innerHTML = offers.map(o => {
      ...
      return `<div class="offer-card" data-id="${o.id}">
        ...
        <div class="oc-actions">
          ...
          <button class="icon-btn oc-del" data-act="del" title="Delete">🗑</button>
        </div>
      </div>`;
    }).join('');
  ...
  function renderDeletedOffers() {
    ...
    host.innerHTML = del.map(o => {
      ...
      return `<div class="del-row" data-id="${o.id}">
        ...
        <button class="btn ghost small" data-act="restore">↩ Restore</button>
        <button class="btn danger small" data-act="purge" aria-label="Delete permanently">🗑</button>
      </div>`;
    }).join('');
  ...
  function renderOfferItems() {
    ...
    host.innerHTML = groups.map(([title, gitems]) => {
      ...
      return `<div class="ogroup" data-gkey="${esc(title)}">
        <div class="ogroup-title">${DRAG_HANDLE}<span>${esc(title) || 'Untitled product'}</span></div>
